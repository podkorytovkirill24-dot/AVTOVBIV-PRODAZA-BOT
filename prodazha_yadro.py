...
def env_first(*names: str) -> str:
    ...
    wanted = {item.strip().upper() for item in names if item}
    for key, value in os.environ.items():
        if not key:
            continue
        if key.strip().upper() in wanted and str(value).strip():
            return str(value).strip()
    return ""
...
class SalesConfig:
    ...
    def from_env(project_root: Path) -> "SalesConfig":
        ...
        if not sales_bot_token:
            raise RuntimeError(
                "Не найден токен sales-бота. "
                "Укажите SALES_BOT_TOKEN (или BOT_TOKEN/TELEGRAM_BOT_TOKEN) в env."
            )

        crypto_pay_token = env_first(
            "CRYPTO_PAY_TOKEN",
